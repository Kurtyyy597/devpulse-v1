...
export const restoreManySoftDeletedSessionsService = (sessionIds: string[], sessions: Session[]) => {
  ...
  const updatedSessions = sessions.map((s) => {
    ...
    if (s.deletedAt === null) {
      throw new AppError("One or more selected sessions have not been soft deleted", 400)
    };
